...
const sexe : string = prompt('Quel est votre sexe (M ou F) ? ')!;
    ...
    } else if (sexe === 'M') { //Si le sexe que l'utilisateur a entré est M alors on dit Bonjour monsier ...
        alert('Bonjour ' + ' monsieur ' + name);
    } else {                    // Dans le cas contraire si l'utilisateur entre F alors on affichera Bonjour madame ... 
